...
class PlanningGraph():
    ...
    def add_literal_level(self, level):
        ...
        for action_node in self.a_levels[level-1]:
            for eff in action_node.effnodes:
                eff.parents.add(action_node)
                action_node.children.add(eff)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for effect_pos in node_a1.action.effect_add:
            if effect_pos in node_a2.action.effect_rem:
                return True

        for effect_neg in node_a1.action.effect_rem:
            if effect_neg in node_a2.action.effect_add:
                return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for precond in node_a2.action.precond_pos:
            if precond in node_a1.action.effect_rem:
                return True

        for precond in node_a2.action.precond_neg:
            if precond in node_a1.action.effect_add:
                return True 
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        return any(par1.is_mutex(par2) for par1 in node_a1.parents for par2 in node_a2.parents)
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            found = False
            for idx, states in enumerate(self.s_levels):
                for s in states:
